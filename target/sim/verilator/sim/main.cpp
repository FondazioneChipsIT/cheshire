...
int main(int argc, char** argv) {
    ...
    bool     llc_ready        = false;  // true once bootrom commits SPM config
    bool     llc_poll_inflight = false; // true while a slink read is in-flight
    ...
    while (!contextp->gotFinish() && !do_exit) {
        ...
        if (!topp->clk_i) {
          ...
          // Poll LLC CFG_SPM_LOW (AmLlc=0x03001000, offset=0x0).
          // Mirrors QuestaSim: slink_poll_bit0(AmLlc + AXI_LLC_CFG_SPM_LOW_OFFSET, ...)
          // The bootrom configures the LLC as all-SPM only AFTER LLC BIST completes.
          // Writing the ELF before that causes writes to go to un-routed memory.
          if (!llc_ready && reset_done && !llc_poll_inflight && cycle >= 100) {
            slink_master->read(0x03001000ULL);
            llc_poll_inflight = true;
          }
          if (!llc_ready && llc_poll_inflight) {
            auto maybe_response = slink_master->get_read_response();
            if (maybe_response) {
          ...
          if (llc_ready && !preload_done_cycle) {
