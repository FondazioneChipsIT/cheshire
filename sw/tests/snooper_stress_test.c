...
int main(void) {
    ...
    extern char dummy1_code_start, dummy1_code_end, dummy2_code_start, dummy2_code_end,
        dummy3_code_start, dummy3_code_end;
    ...
    *reg32(&__base_snprcfg, CFG_REGS_RANGE_2_LAST_L_REG_OFFSET) = (uintptr_t)&dummy3_code_end;
    ...
    // Set core halting level, core is halted if more than 800 unread address elements are present
    // (or more than 4000 instruction elements)
    ...
    *reg32(&__base_regs, CHESHIRE_SCRATCH_0_REG_OFFSET) = 0;
    ...
    for (int i = base; i < last; i = i + 20) {
        ...
        if ((PC_SRC_L <= (uintptr_t)&dummy1_code_start) ||
            (PC_SRC_L >= (uintptr_t)&dummy1_code_end))
    ...
    if (new_last > last) {                             // Snooper didn't recirculate since last read
        for (int i = last; i < new_last; i = i + 20) { // Keep reading normally
            ...
            if ((PC_SRC_L <= (uintptr_t)&dummy2_code_start) ||
                ...
                return 1; // return error in case the buffer contains a PC outside of the logging
                          // region
        }
    ...
    *reg32(&__base_regs, CHESHIRE_SCRATCH_0_REG_OFFSET) = 0;
    ...
    if (new_last > last) {                             // Snooper didn't recirculate since last read
        for (int i = last; i < new_last; i = i + 20) { // Keep reading normally
            ...
            PC_DST_H = *reg32(&__base_snpr, i + 0x0C);
            CTR_TYPE = *reg32(&__base_snpr, i + 0x10);
            if ((PC_SRC_L <= (uintptr_t)&dummy3_code_start) ||
                (PC_SRC_L >= (uintptr_t)&dummy3_code_end))
                return 1; // return error in case the buffer contains a PC outside of the logging
                          // region
        }
    } else { // Snooper recirculated since last read
