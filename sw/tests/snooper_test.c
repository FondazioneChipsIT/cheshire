...
int main(void) {
    ...
    uint32_t instructions[] = {0xf3000017, 0xf3000017, 0x00100013, 0xda678013, 0xf3000017,
                               0x00200013, 0xd9a78013, 0xf3000017, 0x00300013, 0xd8e78013,
                               0xf3000017, 0xf3000017, 0xd8270013, 0xf3000017, 0xd7278013,
                               0xf3000017, 0xd6678013};
    ...
    *reg32(&__base_snprcfg, CFG_REGS_RANGE_0_LAST_L_REG_OFFSET) = (uintptr_t)&dummy1_code_end;
    ...
    // Base is the index of the first valid element (it is incremented starting from 0 only when the
    // buffer starts overwriting itself)
    base = *reg32(&__base_snprcfg, CFG_REGS_BASE_REG_OFFSET);
    // In instruction mode, last is the index of the last valid element + 4 (if no elements are
    // valid this returns zero, if 1 element is valid this returns 4)
    ...
    for (int i = base; i < last; i = i + 4) { // Instruction mode
        if (*reg32(&__base_snpr, i) != instructions[i / 4])
            return 1; // return error in case the logged instruction is different from the expected
                      // instruction
    }
    ...
    *reg32(&__base_snprcfg, CFG_REGS_TRIG_PC0_L_REG_OFFSET) = (uintptr_t)&dummy2_code_end;
    ...
    // The interrupt is high as long as this condition is met and does not stop the snooper
    // operation In instruction mode each element containts a single 32bit field (the instruction
    // opcode) thus the watermark level is increased by 1 for every element logged in instruction
    // mode In address mode each element is composed by 5 32bit fields (PC_SRC_L, PC_SRC_H,
    // PC_DST_L, PC_DST_H, CTR_TYPE) thus the watermark level is increased by 5 for every element
    // logged in address mode

    // Set watermark level to 10 instructions (instruction mode) or 2 instruction addresses (address
    // mode)
    ...
    *reg32(&__base_regs, CHESHIRE_SCRATCH_3_REG_OFFSET) = 3;
    ...
    // In address mode, last is the index of the last valid element + 20 (if no elements are valid
    // this returns zero, if 1 element is valid this returns 20)
    ...
    for (int i = last; i < new_last; i = i + 20) { // Address mode
        ...
        if ((PC_SRC_L <= (uintptr_t)&dummy2_code_start) ||
            (PC_SRC_L >= (uintptr_t)&dummy2_code_end))
